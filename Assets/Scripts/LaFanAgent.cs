...
public class LaFanAgent : Agent
{
    ...
    protected OrientationCubeController p_OrientationCube;

    //The indicator graphic gameobject that points towards the target
    //DirectionIndicator m_DirectionIndicator;
    protected JointDriveController m_JdController;
    EnvironmentParameters m_ResetParams;

    public override void Initialize()
    {
        p_OrientationCube = GetComponentInChildren<OrientationCubeController>();
    ...
    public void CollectObservationBodyPart(BodyPart bp, VectorSensor sensor)
    {
        ...
        sensor.AddObservation(p_OrientationCube.transform.InverseTransformDirection(bp.rb.velocity));
        sensor.AddObservation(p_OrientationCube.transform.InverseTransformDirection(bp.rb.angularVelocity));

        //Get position relative to hips in the context of our orientation cube's space
        sensor.AddObservation(p_OrientationCube.transform.InverseTransformDirection(bp.rb.position));
    ...
    {
        var cubeForward = p_OrientationCube.transform.forward;
        ...
        sensor.AddObservation(p_OrientationCube.transform.InverseTransformDirection(avgVel));
        //avg body vel relative to cube
        //sensor.AddObservation(p_OrientationCube.transform.InverseTransformDirection(avgVel));
        //vel goal relative to cube
        //sensor.AddObservation(p_OrientationCube.transform.InverseTransformDirection(velGoal));
        ...
        sensor.AddObservation(p_OrientationCube.transform.InverseTransformPoint(target.transform.position));
    ...
    void UpdateOrientationObjects()
    {
        ...
        p_OrientationCube.UpdateOrientation(hips, target);
    }

    protected virtual void ComputeReward(){}

    void FixedUpdate()
    {
        UpdateOrientationObjects();

        // Compute the reward at each fixed update
        ComputeReward();

        // terminate episode if body falls under the level 
