...
public class LaFanLegs : Agent, IWalkerAgent
{
    [Header("Walk Speed")]
    [Range(0.1f, 10)]
    [SerializeField]
    ...
    public float coefUpRight = 0.5f;
    public float coefSpeed = 0.5f;
    ...
    public override void Initialize()
    {
        ...
        decisions = GetComponent<DecisionRequester>().DecisionPeriod;
        ...
        // TODO: make the csv a public variable (remove hardcode)
        pose_rotations = Load_Init_Pos.Read_CSV("Text/walk1_subject1_legs_csv");
    ...
    public override void OnEpisodeBegin()
    {
        ...
        // initial pose to a frame of the dataset
        // TODO: preprocess csv animation data to only contain what needed (remove hardcode)
        int pose = Random.Range(75, 450);
        if (pose < 175)
        {
            pose += 75;
        }
        ...
        MTargetWalkingSpeed =
            randomizeWalkSpeedEachEpisode ? Random.Range(1.5f, m_maxWalkingSpeed) : MTargetWalkingSpeed;
    ...
    {
        // add velocities and angular velocities
        ...
        //Get position relative to hips in the context of orientation reference
        sensor.AddObservation(OrientationRef.transform.InverseTransformPoint(bp.rb.transform.position));

        if (counter == 0)
        {
            //if hips pass only the pitch and roll
            sensor.AddObservation(Quaternion.Euler(new Vector3(bp.rb.transform.eulerAngles.x, 0f, bp.rb.transform.eulerAngles.z)));
        }
        else
        {
            // else pass the local rotation
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        sensor.AddObservation(-Vector3.Dot(avgVel, -Vector3.right) * Vector3.right);
        //Position of target position relative to orientation cube
        sensor.AddObservation(Vector3.Dot(hips.up, Vector3.up) * Vector3.up);
    }
    ...
    private void ComputeReward()
    {
        ...
        float uprightReward = Vector3.Dot(Vector3.up, (hips.up+spine.up+spine1.up+spine2.up).normalized);

        var full_reward = coefSpeed * matchSpeedReward + coefUpRight * uprightReward;
        AddReward(full_reward/decisions);

        speedReward = matchSpeedReward;
        this.uprightReward = uprightReward;
    ...
    {
        m_JdController.bodyPartsDict[spine2].rb.mass = m_ResetParams.GetWithDefault("spine2_mass", 5);
        m_JdController.bodyPartsDict[spine1].rb.mass = m_ResetParams.GetWithDefault("spine1_mass", 5);
        m_JdController.bodyPartsDict[spine].rb.mass = m_ResetParams.GetWithDefault("spine_mass", 5);
